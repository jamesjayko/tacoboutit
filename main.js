...
<<<<<<< HEAD
	...
	imgAPICall: function(query, ele) {
		...
		$.ajax(ajaxOptions).then(controller.tacoImageFilter.bind(null, ele, query));
	},
	...
	geocode: function() {
		$.ajax({
			url:
				...
				model.loc +
				"&key=AIzaSyDmBiq2uv9zLd2A1G5KwCbSaUYhMwO6mJg",
			method: "get",
			cache: false,
			dataType: "json",
...
>>>>>>> fe3224504784e1e6eaa6ed1e59a268934c8812ee
};
...
<<<<<<< HEAD
	initiateClickHandlers: function() {
		...
		$(".recipeImage img")[0].onerror = function() {
			this.appendImg($(".recipeImage img"), "https://i.imgur.com/iTuHMPB.png");
		}.bind(this);
	},
		...
		for (var i = 0; i < model.resultsArr.length; i++) {
			...
			if (model.resultsArr[i].hasOwnProperty("photos")) {
				...
				var img = $("<img>")
					.attr(
						"src",
						model.resultsArr[i].photos[0].getUrl({
							maxWidth: 100,
							maxHeight: 100
						})
					)
				...
				var imgContainer = $("<div>").addClass("imgContainer");
				var img = $("<img>")
					.attr("src", "./images/taco_default2.jpg")
					.css({
						maxWidth: 100,
						maxHeight: 100
					})
			...
			if (
				model.resultsArr[i].name.length > 24 &&
				model.resultsArr[i].hasOwnProperty("photos")
			) {
				var storeName = controller.nameTruncate(model.resultsArr[i].name);

				var name = $("<h2>")
					.text(storeName)
					.addClass("name makeMeSmaller");
				elementsList.push(name);
			} else {
				var name = $("<h2>")
					.text(model.resultsArr[i].name)
					.addClass("name");
				elementsList.push(name);
			}
			if (model.resultsArr[i].hasOwnProperty("rating")) {
				var rating = $("<div>")
					.text(model.resultsArr[i].rating)
					.addClass("rating");
				elementsList.push(rating);
			}
			if (model.resultsArr[i].hasOwnProperty("simonsData")) {
				if (
					model.resultsArr[i].simonsData.hasOwnProperty(
						"formatted_phone_number"
					)
				) {
					var phoneNumber = $("<h4>")
						.text(model.resultsArr[i].simonsData.formatted_phone_number)
	...
	tacoImageFilter: function(ele, query, data) {
		...
		for (var qI = 0; qI < qArray.length; qI++) {
			if (qArray[qI].title.indexOf("aco") !== -1) {
				view.appendImg(
					ele,
					`https://farm${qArray[qI].farm}.staticflickr.com/${
						qArray[qI].server
					}/${qArray[qI].id}_${qArray[qI].secret}.jpg`
				);
				photoFound = true;
			}
		}
	...
	gleanRecipe: function(recipe) {
		...
		for (let i = 0; i < gleanedRecipe.length; i++) {
			if (gleanedRecipe[i] === "") {
				...
				gleanedRecipe[i] = gleanedRecipe[i].substr(
					0,
					gleanedRecipe[i].indexOf("(")
				);
			}
		}
